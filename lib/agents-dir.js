...
function parseFrontmatterValues(lines, close) {
    const values = {};
 ...
 */
export function parseAgentMarkdown(text, path) {
    ...
    if (name === undefined || name === '')
        return { ok: false, error: 'missing required frontmatter key `name`' };
    let deep = 1;
    ...
    if (maxRounds !== undefined && maxRounds !== '') {
        ...
        // a cap the author wrote but that never applies is worse than a visible
        // broken file.
...
export function listAgentFiles(dir) {
    const agents = [];
    const broken = [];
    if (!existsSync(dir))
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (result.ok)
            ...
            broken.push({ path, error: result.error });
    }
