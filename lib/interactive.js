...
import { listAgentFiles } from "./agents-dir.js";
import { accountingTurnEnd, readStoredEvents, toStopReason, } from "./resume.js";
import { loadAgent, stopReasonError, textOf } from "./tool-run-agent.js";
// ---------------------------------------------------------------------------
 ...
 */
export function decideBackgroundMode(explicit, frontmatter) {
 ...
 */
export function pickLatestContinuableChild(entries, labels) {
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (entry.mode !== 'continuable')
            continue;
        if (entry.label === undefined || !labels.includes(entry.label))
            continue;
        return entry;
    }
    return undefined;
 ...
 */
export function getLiveSession(ctx, childId) {
 ...
 */
export async function childEventBoundary(ctx, childId, signal) {
 ...
 */
async function readEventsAfter(ctx, childId, boundary, signal) {
    const live = getLiveSession(ctx, childId);
    if (live !== undefined) {
        try {
 ...
 */
export function askAgentTool(ctx, cfg) {
    ...
    const description = `Send a follow-up message to a background custom agent and WAIT for its reply. ` +
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (hasName === hasId) {
                throw new Error(`${cfg.toolName}: pass exactly one of "agent" (an agent name) or "agent_id" (the durable subagent id a background dispatch returned)`);
            }
            let childId;
            if (hasId) {
                childId = args.agent_id;
            }
            else {
                const available = agents.map((agent) => agent.meta.name);
                const agent = loadAgent(cfg.agentsDir, args.agent, available);
                ...
                try {
                    ...
                    throw new Error(`${cfg.toolName}: cannot enumerate this conversation's subagent children (${String(error)}) — ` +
                ...
                if (candidate === undefined) {
                    throw new Error(`${cfg.toolName}: no background run of agent "${args.agent}" was found in this conversation — ` +
            ...
            let messageId;
            try {
                messageId = await ctx.subagents.sendMessage(parent, childId, [{ type: 'text', text: args.message }], { signal: exec.signal });
            }
            catch (error) {
                if (error instanceof SubagentError && error.code === 'NOT_RESUMABLE') {
                    throw new Error(`${cfg.toolName}: subagent ${String(childId)} is not continuable — only background dispatches ` +
                        `(${dispatchTool} with background: true) accept follow-ups; one-shot runs do not. ` +
                        `Re-dispatch the agent with background: true to start an interactive conversation.`, { cause: error });
                }
                throw error;
            }
            if (boundary === undefined) {
                return replyOutput([
                    {
                        type: 'text',
                        text: `Message delivered to subagent ${String(childId)} (message ${String(messageId)}); ` +
            ...
            if (reply.stopReason !== 'completed') {
                const text = textOf(reply.output);
