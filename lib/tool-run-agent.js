...
import { getEffortRegistry } from "./effort-inject.js";
 ...
 */
async function settleRegisteredForegroundRun(run, agentName, thinking) {
    ...
    registry?.register(run.id, thinking);
    try {
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (decision === 'resume' && candidate !== undefined) {
                ...
                const registry = thinking === undefined ? undefined : getEffortRegistry();
                registry?.register(candidate.childId, thinking);
                try {
                    let resumed;
                    try {
                        resumed = await driveResumedRun({
                            ...
                            toolFilter: agent.meta.deep === 0
                                ...
                                : undefined,
                            agentOptions: modelOptions.provider !== undefined || modelOptions.model !== undefined
                                ? modelOptions
                                : undefined,
