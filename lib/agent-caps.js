 ...
 */
export function readAgentMaxRounds(label, dir) {
    const name = label.trim();
    if (name === '')
        return undefined;
    const { agents } = listAgentFiles(dir);
    const byName = agents.find((agent) => agent.meta.name === name);
    if (byName !== undefined)
        return byName.meta.maxRounds;
    const byDisplay = agents.filter((agent) => agent.meta.displayName === name);
    if (byDisplay.length === 1)
        return byDisplay[0].meta.maxRounds;
    return undefined;
}
//# sourceMappingURL=agent-caps.js.map