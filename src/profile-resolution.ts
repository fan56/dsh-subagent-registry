...
import { dshHome, THINKING_LEVELS } from './agents-dir.ts'
 ...
 */
function parsePinName(text: string): string | undefined {
 ...
 */
export function workspaceProfileName(startDir?: string): string | null {
 ...
 */
export function readModelProfilesDoc(): unknown | null {
  try {
...
function baselineFrom(
  base: { model?: string | null; thinking?: string | null },
): { model?: string; thinking?: string } {
  const result: { model?: string; thinking?: string } = {}
  if (typeof base.model === 'string' && base.model !== '') result.model = base.model
  if (typeof base.thinking === 'string' && isThinkingLevel(base.thinking)) result.thinking = base.thinking
  return result
}
 ...
 * the baseline.
 */
export function composeAgentRuntime(
  agentName: string,
  base: { model?: string | null; thinking?: string | null },
  ...
  if (pinName === null) return effective
  const doc = readModelProfilesDoc()
  if (!isRecord(doc)) return effective
  if (!Array.isArray(doc.profiles)) return effective
  ...
  if (profile === undefined) return effective
  if (!isRecord(profile.agents)) return effective
  const entry = profile.agents[agentName]
  if (!isRecord(entry)) return effective
  const model = stringField(entry, 'model')
  if (model !== undefined) effective.model = model
  const thinking = stringField(entry, 'thinking')
  if (thinking !== undefined && isThinkingLevel(thinking)) effective.thinking = thinking
  return effective
}
