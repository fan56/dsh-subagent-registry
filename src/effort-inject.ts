...
// Type-only side-effect import: pulls @deepseek-ai/dsh-agent's `Events`
// augmentation into the program so the `'agent/request'` listener below is
// fully typed (payload + waterfall `next`) rather than falling back to an
// untyped event key. Erased at emit; no runtime dependency.
...
let activeRegistry: EffortRegistry | undefined
 ...
 */
export function getEffortRegistry(): EffortRegistry | undefined {
 ...
 */
export function installEffortInjection(ctx: Context): EffortRegistry {
  const efforts = new Map<string, ReasoningEffortId>()
